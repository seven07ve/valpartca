...
$ssql=sprintf("SELECT * FROM contenido WHERE id_cont='".$_GET["num"]."'");
...
?>
      ...
      <input type="hidden" name="id_cont" value="<?php echo $id_cont ?>" />
        ...
        <label>Usuario actual<br />
          <input name="usuario-actual" type="text" disabled="disabled" id="autor" value="<?php echo $_SESSION['user_cuenta'] ?>" size="60" maxlength="100"  />
          <input name="autor" type="hidden" id="autor" value="<?php echo $_SESSION['user_cuenta'] ?>"  />
        </label><br /><br />
	...
	<form action="ins_img-bg.php" method="post" enctype="multipart/form-data" name="form2" id="form2" style="margin-bottom:30px;">
        ...
        <input type="hidden" name="bgimg" value="<?php echo $id_cont ?>" />
        <input type="hidden" name="pagina" value="modificar.php?num=<?php echo $id_cont ?>" />
        <p>
          <label>
            <input name="repetir" type="radio" id="0" value="0" />
            No Repetir</label>
          <label>
          <label>
            <input type="radio" name="repetir" value="1" id="1" />
            Repetir</label>
          <label>
            <input type="radio" name="repetir" value="2" id="2" />
            Repetir Horizontalmente</label>
          <label>
            <input type="radio" name="repetir" value="3" id="3" />
            Repetir Verticalmente</label>
          <br />
        </p>
<input type="file" name="img" id="fileField2" style="margin:0 0 15px" />
...
Foreach ($ficheros1 as $valor)
{
	if ($valor == '.' or $valor == '..'){
		...
		echo '<input type="hidden" name="pagina" value="modificar.php?num='.$id_cont.'" />';
		echo '<input type="submit" id="submit" value="Eliminar" /></form>';
	}
}
?>      
	<div style="width:auto; height:300px;"></div><br clear="all" />
    </div>
