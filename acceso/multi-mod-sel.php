...
if (!$_POST["ano"] and !$_POST["mes"]){
	$ssql=sprintf("SELECT titulo, id_multimedia FROM multimedia WHERE ano='".date("Y")."' AND mes='".date("m")."' ORDER BY id_multimedia DESC");
}
else if ($_POST["ano"] == "" and $_POST["mes"]){
	$ssql=sprintf("SELECT titulo, id_multimedia FROM multimedia WHERE ano='".date("Y")."' AND mes='".$_POST["mes"]."' ORDER BY id_multimedia DESC");
}
else if ($_POST["ano"] and $_POST["mes"] == ""){
	$ssql=sprintf("SELECT titulo, id_multimedia FROM multimedia WHERE ano='".$_POST["ano"]."' AND mes='".date("m")."' ORDER BY id_multimedia DESC");
}
else{
	$ssql=sprintf("SELECT titulo,id_multimedia FROM multimedia WHERE ano='".$_POST["ano"]."' AND mes='".$_POST["mes"]."' ORDER BY id_multimedia DESC");
}
...
?>
  ...
  <div class="contenedor">
    <h2>Modificar Artículo Multimedia</h2>
    <br />
    <br />
          ...
          <select name="jumpMenu" id="jumpMenu" onchange="MM_jumpMenu('parent',this,0)">
            <option selected>Seleccione</option>
                ...
                while ($fila = $resultado->fetch_array(MYSQLI_ASSOC)){
                    echo '<option value="multi-mod.php?num='.$fila["id_multimedia"].'">'.$fila["titulo"].'</option>"';
				}
                ?>                      
          </select>
