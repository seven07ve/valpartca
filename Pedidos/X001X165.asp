...
<SCRIPT LANGUAGE="javascript" type="text/javascript">
	...
	wTIT = "Deportes / Hobbies / Procedencia"
	wCLA001 = trim(Request.QueryString("CLA001"))
    ...
    IF len(wCLA001) > 0 then	 
		...
		SQL = "SELECT * FROM X300PF700 " 
		SQL = SQL & " WHERE "
		SQL = SQL & " P300700CCI = '" & wCCI & "' AND "
		SQL = SQL & " P300700CON ='" &  wCLA001 & "'" 	

		wACT = ""
		Set rstQuery = Server.CreateObject("ADODB.Recordset")
		rstQuery.Open sql , strCnn
		if NOT rstQuery.EOF then
			wDEL = trim(rstQuery.Fields("P300700DES"))
			wTIP = trim(rstQuery.Fields("P300700TIP"))
			wPUS = trim(rstQuery.Fields("P300700PUS"))
			wFAC = trim(rstQuery.Fields("P300700FAC"))
	  ...
	  <table BORDER="0" WIDTH="100%" cellpadding="5" >
		 ...
		 <tr>
          ...
          <select name="TIP">
			...
			<OPTION value="1" <%=wSEL1%> >Hobbie</OPTION>	  
			<OPTION value="2" <%=wSEL2%> >Deporte</OPTION>	  
			<OPTION value="3" <%=wSEL3%> >Procedencia</OPTION>	  
		</optgroup>
		</select>       
		 </td>
         </tr>
