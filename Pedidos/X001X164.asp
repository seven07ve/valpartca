...
<SCRIPT LANGUAGE="javascript" type="text/javascript">
    ...
    IF len(wCLA001) > 0 then	 
		...
		SQL = "SELECT * FROM X300PF705 " 
		SQL = SQL & " WHERE "
		SQL = SQL & " P300705CCI = '" & wCCI & "' AND "
		SQL = SQL & " P300705COD ='" &  wCLA001 & "'" 	

		wACT = ""
		Set rstQuery = Server.CreateObject("ADODB.Recordset")
		rstQuery.Open sql , strCnn
		if NOT rstQuery.EOF then
			wDEL = trim(rstQuery.Fields("P300705DES"))
			wCRE = trim(rstQuery.Fields("P300705CRE"))
			wPUS = trim(rstQuery.Fields("P300705PUS"))
			wFAC = trim(rstQuery.Fields("P300705FAC"))
   ...
   <table align="center" BORDER="1" WIDTH="650" cellspacing="0" cellpadding="0"  bordercolor="#FFFF" bgcolor="<%=Request.Cookies("CAR")("Color2")%>" style="border-style: inset; border-width: 1px; padding-left: 1px; padding-right: 1px; padding-top: 1px; padding-bottom: 1px">
	  ...
	  <table BORDER="0" WIDTH="100%" cellpadding="5" >
		 ...
		 <tr>
		  ...
		  <tr>
          <td class="TDS" width="20%" ><p CLASS="P1" align="left">Regional:</td>
          <td class="TDD" width="80%" >
          <select name="CRE">
			<optgroup label="Por favor seleccione">
		 <%         
				SQL = " SELECT P300704CRE, P300704DES FROM X300PF704 " 																	
 				SQL = SQL & " WHERE "
				SQL = SQL & " P300704CCI = '" & trim(Session("glCiaInternet")) & "' "
 				SQL = SQL & " ORDER BY  P300704DES " 					
				Set rstQuery = Server.CreateObject("ADODB.Recordset")
				rstQuery.Open sql , strCnn
                DO WHILE NOT rstQuery.EOF 
					wCOD = trim(rstQuery.Fields(0))
					wDES = trim(rstQuery.Fields(1))							
					IF TRIM(wCRE) = wCOD THEN
					   wSEL = " selected"
