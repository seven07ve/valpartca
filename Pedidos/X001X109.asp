...
<SCRIPT LANGUAGE="javascript" type="text/javascript">
	...
	function cmdSubmit_onclick(){
		if (document.frmGral.wCLA.value.length==0){ 
			alert('Debe Indicar el Codigo del Banco.');
    ...
    IF len(wCLA001) > 0 then	 
		...
		SQL = "SELECT * FROM X300AF025 " 
		SQL = SQL & " WHERE "
		SQL = SQL & " A300025CCI = '" & wCCI & "' AND "
		SQL = SQL & " A300025CIA = '" & wCIA & "' AND "		
		SQL = SQL & " A300025CBN ='" &  wCLA001 & "'" 	

		wACT = ""
		Set rstQuery = Server.CreateObject("ADODB.Recordset")
		rstQuery.Open sql , strCnn
		if NOT rstQuery.EOF then
			wDEL = trim(rstQuery.Fields("A300025DEL"))
			wPUS = trim(rstQuery.Fields("A300025PUS"))
			wFAC = trim(rstQuery.Fields("A300025FAC"))
...
<br>
<br>
   <table align="center" BORDER="1" WIDTH="650" cellspacing="0" cellpadding="0"  bordercolor="#ffffff" bgcolor="<%=Request.Cookies("CAR")("Color2")%>" style="border-style: inset; border-width: 2px; padding-left: 2px; padding-right: 2px; padding-top: 1px; padding-bottom: 1px">
	  <TR>
	  <td>
